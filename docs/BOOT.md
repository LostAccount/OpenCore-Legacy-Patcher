...
Reboot the machine while holding `Option` (labeled `alt` in some regions) to select the EFI Boot entry with the OpenCore icon (holding the `Control` key will make this the default boot entry):
